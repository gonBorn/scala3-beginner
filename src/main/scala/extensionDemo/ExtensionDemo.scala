...
object ExtensionDemo:
  case class Person(name: String, age: Int):
    ...
    def verify[M[_]: Sync]: M[Person] =
      val value: Validated[CustomError.UnsatisfiedPerson, Person] =
        (this.validatedGet(validateName), this.validatedGet(validateAge))
          .mapN(Person.apply)
  ...
  extension (p: Person) def generateId: Int = p.age + 1
